...
export class DataHoraParser extends DataHoraBaseParser {

    public static parseDataHora(date: string, config: any, fromPipe: boolean, exibirSegundos:boolean, triggerOnBlur?: boolean): any {
        if (!config || !config.config) {
            return date;
        }
        let pattern = exibirSegundos ? config.config.dataHora.patternComSegundos : config.config.dataHora.pattern;
        //
